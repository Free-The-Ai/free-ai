...
const targets = computed<Record<string, Surface>>(() => {
    ...
    for (const key of ctx.configKeys) {
        ...
        const floor = band.map((b, i) =>
            line ? Math.min(nRows - 1, top[i] + glow) : (ctx.y(b[0]) / h) * (nRows - 1),
        );
...
const stars = computed<Star[]>(() => {
    const out: Star[] = [];
    const per = Math.max(4, Math.round(cols.value / 14));
    ...
    const draw = (now: number) => {
        ...
        const mx =
            marker != null && s.dataLength > 1
                ? Math.round((marker / (s.dataLength - 1)) * (nCols - 1))
                : -1;
...
function restart(): void {
    ...
    if (!canvas) return;
    stop = startLoop(canvas, bloomRef.value, cols.value, rows.value);
}
onMounted(restart);
watch([cols, rows], restart);
