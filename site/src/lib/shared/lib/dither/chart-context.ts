// Context keys, context value types, and boundary-guard helpers for the
// dither chart engine. The reactive controllers that build these values live in
// the root components (CartesianRoot.svelte / PolarRoot.svelte) — Svelte's
// natural home for provided state — so this module stays framework-glue only.
...
export const ChartKey = Symbol("dk-chart");
export const CommonKey = Symbol("dk-common");
export const SeriesKey = Symbol("dk-series");
export const PolarKey = Symbol("dk-polar");
...
export function useChart(): ChartContextValue {
    const ctx = getContext<ChartContextValue | undefined>(ChartKey);
...
export function useCommonChart(): CommonChart {
    const ctx = getContext<CommonChart | undefined>(CommonKey);
...
export function useSeries(part: string): SeriesContextValue {
    const ctx = getContext<SeriesContextValue | undefined>(SeriesKey);
...
export function usePolarChart(): PolarChartContextValue {
    const ctx = getContext<PolarChartContextValue | undefined>(PolarKey);
