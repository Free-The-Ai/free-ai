/**
 * Sound system singleton — plain module-level state, framework-agnostic by
 * design so it works the same whether it is called from a Vue component,
 * a router hook, or a bare DOM event listener. No Vue provide/inject wiring
 * needed: every caller imports the same functions from the same ES module.
 *
...
export function initSoundSystem(): void {
  ...
  // Expose sound API globally for app/interactions.ts and inline handlers
  // (mute toggle, copy-button feedback, etc.).
