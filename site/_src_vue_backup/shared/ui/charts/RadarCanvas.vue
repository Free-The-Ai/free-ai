...
function restart(): void {
    ...
    if (!canvas) return;
    stop = startLoop(canvas, bloomRef.value, cols.value, rows.value, ctx.plot.width, ctx.plot.height);
}
onMounted(restart);
watch([cols, rows, () => ctx.plot.width, () => ctx.plot.height], restart);
...
const bloomStyle = computed<CSSProperties>(() => {
    ...
    return { ...posStyle.value, transition: "opacity 220ms ease", ...((bloom ?? { opacity: 0 }) as CSSProperties) };
});
</script>
