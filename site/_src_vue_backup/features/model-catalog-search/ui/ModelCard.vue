<script setup lang="ts">
import { computed } from "vue";
import type { Model } from "@/entities/model";
import { modelContext, modelSupportsAudio, modelSupportsImage } from "@/entities/model";
import { formatTokens } from "@/shared/lib/format";

const props = defineProps<{ model: Model }>();
const emit = defineEmits<{ select: [Model] }>();

const ctx = computed(() => modelContext(props.model));
const out = computed(() => props.model.max_output_tokens);
const isAudio = computed(() => modelSupportsAudio(props.model));
const isImage = computed(() => modelSupportsImage(props.model));

function onSelect(event: Event): void {
    if ((event.target as HTMLElement).closest(".model-copy")) return;
    emit("select", props.model);
}

function onKeydown(event: KeyboardEvent): void {
    if (event.key === "Enter" || event.key === " ") {
        event.preventDefault();
        emit("select", props.model);
    }
}

function onCopy(event: MouseEvent): void {
    event.preventDefault();
    event.stopPropagation();
    navigator.clipboard.writeText(props.model.id).catch((error) => {
        console.error("Failed to copy model alias", error);
    });
    const btn = event.currentTarget as HTMLElement;
    const icon = btn.querySelector(".material-symbols-outlined");
    if (icon) {
        icon.textContent = "check";
        setTimeout(() => {
            icon.textContent = "content_copy";
        }, 1500);
    }
}
</script>

<template>
    <article
        :class="['model-card', { 'is-gated': model.requires_seems_legit }]"
        role="button"
        tabindex="0"
        aria-haspopup="dialog"
        :aria-label="`Open details for ${model.id}`"
        data-sound="interaction.tap"
        @click="onSelect"
        @keydown="onKeydown"
    >
        <div class="model-card-top">
            <span class="model-prefix">{{ model.prefix }}/*</span>
        </div>
        <code class="model-id">{{ model.id }}</code>
        <div class="model-meta">
            <span v-if="ctx > 0" class="model-chip" title="Total context window">
                <strong>{{ formatTokens(ctx) }}</strong> ctx
            </span>
            <span v-if="out !== undefined" class="model-chip" title="Maximum output tokens">
                <strong>{{ formatTokens(out) }}</strong> out
            </span>
            <span v-if="isAudio" class="model-chip is-audio" title="Supports an audio route">
                <span class="material-symbols-outlined model-chip-icon" aria-hidden="true">graphic_eq</span>
                Audio
            </span>
            <span v-if="isImage" class="model-chip is-images" title="Supports an image route">
                <span class="material-symbols-outlined model-chip-icon" aria-hidden="true">image</span>
                Image
            </span>
        </div>
        <button class="model-copy" title="Copy model alias" :aria-label="`Copy ${model.id}`" data-sound="interaction.confirm" @click="onCopy">
            <span class="material-symbols-outlined">content_copy</span>
        </button>
    </article>
</template>
